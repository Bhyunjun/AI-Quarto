...
class MCTS:
    ...
    def search(self, initial_state, itermax=1000, max_depth=10, time_limit=20.0):
        """
        MCTS 탐색 실행:
        - 최대 반복(itermax)과 시간 제한(time_limit)을 모두 고려하여 탐색 종료.
        """
        ...
        print(f"Selected Best Child: Action: {best_child.action}, Visits: {best_child.visits}, Value: {best_child.value}")
        print("====================================")
    ...
    def _simulate(self, state, max_depth=8):
        ...
        if current_state.is_terminal():
            ...
            # 최대 깊이에 도달하면 휴리스틱 평가
            sc = self.heuristic_evaluation(current_state)
            if state.inturn == 0:
                return sc if state.turn == 0 else -sc
            elif state.inturn == 1:
                return sc if state.turn == 2 else -sc
    ...
    def heuristic_evaluation(self, state):
        ...
        score += self.evaluate_piece_diversity(state)  # 말 다양성 감소
    ...
    def evaluate_opponent_threat(self, state):
        ...
        for line in lines:
            if 0 in line:  # 비어있는 칸이 있는 경우
                continue
            attributes = [state.all_pieces[int(cell) - 1] for cell in line if cell != 0]
            for i in range(4):  # 4개의 속성 각각에 대해 평가
                if all(attr[i] == attributes[0][i] for attr in attributes):  # 동일 속성 정렬
                    score += 0.2

        return score

    def evaluate_piece_diversity(self,state):
        """
        남은 말의 다양성을 감소시키는 말을 우선 선택.
        """
        diversity_score = 0
        for piece in state.remaining_pieces:
            attributes = list(zip(*state.remaining_pieces))  # 속성별 그룹화
            for i in range(4):  # 4개의 속성 각각에 대해 평가
                if attributes[i].count(piece[i]) == 1:  # 특정 속성이 유일하다면
                    diversity_score -= 0.2
